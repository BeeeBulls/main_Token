...
describe("spl-token-minter", () => {
  ...
  const tokenTitle = "Bee Bull";
  const tokenSymbol = "BB";
  const tokenUri =
    "https://raw.githubusercontent.com/BeeeBulls/officaial_imag/refs/heads/main/tokenLogo.png";
  ...
  it("Mint some tokens to your wallet!", async () => {
    ...
    const tx = await program.methods
      .mintTo(
        new anchor.BN(1025000000000) // amount to mint with zeros same as the number of token decimals
      )
      .accounts({ 
        mintAuthority: wallet.publicKey,
